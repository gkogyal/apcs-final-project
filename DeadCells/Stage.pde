  ...
  public void addRoom(Room r, PVector coord) {
    ...
    for (int row = 0; row < r.map.length; row++)
      for (int col = 0; col < r.map[row].length; col++)
        if ( r.map[row][col] ) map[(int)coord.y + row][(int)coord.x + col] = true;
    ...
    for (int i = 0; i < r.connections.size(); i++) {
      ...
      if (attempts <= 25) addRoom(t, new PVector(coord.x + r.connections.get(i).x - t.connections.get(correctConnect).x, coord.y + r.connections.get(i).y - t.connections.get(correctConnect).y));
    }
  }
